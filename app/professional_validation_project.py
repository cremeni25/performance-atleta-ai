...
from app.project_access import require_project_professional
...
def _result(resultado_id: UUID) -> dict[str, Any]:
    rows = _request("GET", "/rest/v1/agp_resultados_analiticos", params={"id": f"eq.{resultado_id}", "select": "id,status,projeto_id", "limit": "1"})
    if not rows:
...
def list_professional_results(projeto_id: UUID, authorization: str | None = Header(default=None)) -> list[dict[str, Any]]:
    require_project_professional(authorization, projeto_id)
    rows = _request("GET", "/rest/v1/agp_resultados_profissionais_operacionais", params={"projeto_id": f"eq.{projeto_id}", "select": "*", "order": "created_at.desc"})
    return rows if isinstance(rows, list) else []


@router.post("/resultados/{resultado_id}/validacoes", status_code=status.HTTP_201_CREATED)
def validate_result(resultado_id: UUID, payload: ProfessionalValidationInput, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    result = _result(resultado_id)
    professional_id = require_project_professional(authorization, UUID(str(result["projeto_id"])))
    ...
    return _single_row(_request("POST", "/rest/v1/agp_validacoes_profissionais", payload={
        "resultado_id": str(resultado_id),
        "decisao": payload.decisao,
        "parecer_tecnico": payload.parecer_tecnico,
        "papel_profissional": payload.papel_profissional,
        "profissional_auth_id": str(professional_id),
...
def list_result_validations(resultado_id: UUID, authorization: str | None = Header(default=None)) -> list[dict[str, Any]]:
    result = _result(resultado_id)
    require_project_professional(authorization, UUID(str(result["projeto_id"])))
    rows = _request("GET", "/rest/v1/agp_validacoes_profissionais", params={"resultado_id": f"eq.{resultado_id}", "select": "*", "order": "created_at.desc"})
    return rows if isinstance(rows, list) else []
