...
def _resolve_participant(participante_id: UUID) -> tuple[dict[str, Any], dict[str, Any], UUID]:
    ...
    profile = _first(_request("GET", "/rest/v1/agp_perfis_esportivos", params={
        ...
        "status": "eq.ativo",
        "select": "id,pessoa_id,legacy_perfil_atleta_id,modalidade,prova_posicao,categoria,nivel",
        "limit": "1",
    })) or {}
...
def _resolve_participant_from_intervention(intervention: dict[str, Any]) -> dict[str, Any]:
    profile = _first(_request("GET", "/rest/v1/agp_perfis_esportivos", params={
        "legacy_perfil_atleta_id": f"eq.{intervention['atleta_id']}",
        "status": "eq.ativo",
        "select": "pessoa_id",
        "limit": "1",
    }))
    if not profile:
        raise HTTPException(status_code=422, detail="Perfil esportivo da intervenção não encontrado")
    participant = _first(_request("GET", "/rest/v1/agp_participantes_projeto", params={
...
def _synthesis(assessments, results, interventions, responses, sessions, collections) -> dict[str, Any]:
    ...
    for domain, item in latest.items():
        restrictions.extend({"dominio": domain, "origem": item.get("papel_profissional"), "conteudo": value} for value in (item.get("restricoes") or []))
        recommendations.extend({"dominio": domain, "origem": item.get("papel_profissional"), "conteudo": value} for value in (item.get("recomendacoes") or []))

    validated_results = [r for r in results if r.get("status") == "validado"]
    active_interventions = [i for i in interventions if i.get("status") in {"proposta", "aprovada", "em_execucao"}]
    latest_response = responses[0] if responses else None

    if restrictions:
        state, next_action = "restricao_profissional_presente", "Revisar restrições profissionais antes de alterar carga, conteúdo ou disponibilidade competitiva."
    elif active_interventions and not latest_response:
        state, next_action = "intervencao_sem_resposta_mensurada", "Mensurar a resposta da intervenção com a mesma referência usada na decisão inicial."
    elif latest_response and latest_response.get("classificacao_resposta") == "piora":
        state, next_action = "resposta_desfavoravel", "Reavaliar intervenção e contexto multidisciplinar antes de mantê-la."
    elif latest_response and latest_response.get("classificacao_resposta") == "melhora":
        state, next_action = "resposta_favoravel", "Confirmar sustentação do efeito e decidir continuidade, progressão ou retirada da intervenção."
    elif validated_results:
        state, next_action = "resultado_validado_sem_intervencao", "Converter o resultado validado em ação rastreável quando houver justificativa profissional."
    elif assessments:
        state, next_action = "avaliacoes_em_consolidacao", "Cruzar avaliações profissionais com treino, recuperação e evidências longitudinais antes da intervenção."
    else:
        state, next_action = "sem_avaliacao_multidisciplinar", "Registrar apenas avaliações profissionais realmente pertinentes ao atleta."
    ...
    return {
        ...
        "nota": "Cobertura não é meta administrativa: só domínios pertinentes devem ser avaliados.",
    }


@router.get("/participantes/{participante_id}/ciclo-decisao")
def get_decision_cycle(participante_id: UUID, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    _require_owner(authorization)
    participant, profile, athlete_id = _resolve_participant(participante_id)
    assessments = _rows(_request("GET", "/rest/v1/agp_avaliacoes_profissionais", params={"participante_id": f"eq.{participante_id}", "select": "*", "order": "data_avaliacao.desc", "limit": "100"}))
    results = _rows(_request("GET", "/rest/v1/agp_resultados_analiticos", params={"atleta_id": f"eq.{athlete_id}", "select": "*", "order": "created_at.desc", "limit": "30"}))
    interventions = _rows(_request("GET", "/rest/v1/agp_intervencoes", params={"atleta_id": f"eq.{athlete_id}", "select": "*", "order": "created_at.desc", "limit": "50"}))
    responses = _rows(_request("GET", "/rest/v1/agp_respostas_intervencao", params={"atleta_id": f"eq.{athlete_id}", "select": "*", "order": "data_avaliacao.desc", "limit": "50"}))
    sessions = _rows(_request("GET", "/rest/v1/agp_sessoes_treinamento", params={"atleta_id": f"eq.{athlete_id}", "select": "*", "order": "data_hora_inicio.desc", "limit": "50"}))
    collections = _rows(_request("GET", "/rest/v1/agp_coletas", params={"participante_id": f"eq.{participante_id}", "select": "id,status,data_hora_coleta,instrumento_id,protocolo_id,dados,confiabilidade,completude", "order": "data_hora_coleta.desc", "limit": "100"}))
    result_ids = ",".join(str(r["id"]) for r in results)
    validations = _rows(_request("GET", "/rest/v1/agp_validacoes_profissionais", params={"resultado_id": f"in.({result_ids})" if result_ids else "eq.00000000-0000-0000-0000-000000000000", "select": "*", "order": "created_at.desc", "limit": "100"}))
    documents = _rows(_request("GET", "/rest/v1/agp_documentos_profissionais", params={"atleta_id": f"eq.{athlete_id}", "select": "*", "order": "data_documento.desc", "limit": "50"}))
    return {
        "versao_nucleo": "agp-multidisciplinary-decision-v1.1",
...
def create_professional_assessment(participante_id: UUID, payload: ProfessionalAssessmentInput, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    ...
    return _single_row(_request("POST", "/rest/v1/agp_avaliacoes_profissionais", payload={
        "participante_id": str(participante_id), "atleta_id": str(athlete_id), "projeto_id": participant["projeto_id"],
        "dominio": payload.dominio, "papel_profissional": payload.papel_profissional, "profissional_auth_id": str(owner_id),
        "instrumento_referencia": payload.instrumento_referencia, "metricas": payload.metricas, "achados": payload.achados,
        "restricoes": payload.restricoes, "recomendacoes": payload.recomendacoes, "confianca": payload.confianca,
        "validade_ate": payload.validade_ate.isoformat() if payload.validade_ate else None, "status": "validada",
    }), "avaliação profissional")


@router.post("/participantes/{participante_id}/intervencoes", status_code=status.HTTP_201_CREATED)
def create_intervention(participante_id: UUID, payload: InterventionInput, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    owner_id = _require_owner(authorization)
    participant, _profile, athlete_id = _resolve_participant(participante_id)
    if payload.dominio not in DOMAINS:
    ...
    return _single_row(_request("POST", "/rest/v1/agp_intervencoes", payload={
        "atleta_id": str(athlete_id), "projeto_id": participant["projeto_id"], "dominio": payload.dominio,
        "responsavel_auth_id": str(owner_id), "papel_responsavel": payload.papel_responsavel, "descricao": payload.descricao,
        "justificativa": payload.justificativa, "evidencia_origem": payload.evidencia_origem,
        "resultado_esperado": payload.resultado_esperado, "inicio": datetime.now(timezone.utc).isoformat(),
        "fim_previsto": payload.fim_previsto.isoformat() if payload.fim_previsto else None, "status": payload.status,
    }), "intervenção")


@router.post("/intervencoes/{intervencao_id}/respostas", status_code=status.HTTP_201_CREATED)
def create_intervention_response(intervencao_id: UUID, payload: InterventionResponseInput, authorization: str | None = Header(default=None)) -> dict[str, Any]:
    owner_id = _require_owner(authorization)
    intervention = _first(_request("GET", "/rest/v1/agp_intervencoes", params={"id": f"eq.{intervencao_id}", "select": "*", "limit": "1"}))
    if not intervention:
        raise HTTPException(status_code=404, detail="Intervenção não encontrada")
    participant = _resolve_participant_from_intervention(intervention)
    ...
    response = _single_row(_request("POST", "/rest/v1/agp_respostas_intervencao", payload={
        "intervencao_id": str(intervencao_id), "participante_id": participant["id"], "atleta_id": intervention["atleta_id"],
        "projeto_id": intervention["projeto_id"], "avaliado_por_auth_id": str(owner_id), "papel_avaliador": payload.papel_avaliador,
        ...
        "metricas_antes": payload.metricas_antes, "metricas_depois": payload.metricas_depois, "variacoes": variations,
        "classificacao_resposta": payload.classificacao_resposta, "confianca": payload.confianca,
        "conclusao": payload.conclusao, "recomendacao_proximo_ciclo": payload.recomendacao_proximo_ciclo,
    }), "resposta da intervenção")
    new_status = "concluida" if payload.classificacao_resposta in {"melhora", "neutra", "piora"} else "em_execucao"
