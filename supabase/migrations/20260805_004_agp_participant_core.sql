...
-- Migração aditiva: preserva perfis_atletas e todas as estruturas existentes.
...
create unique index if not exists agp_pessoas_email_uidx
...
create table if not exists public.agp_perfis_esportivos (
  id uuid primary key default gen_random_uuid(),
  pessoa_id uuid not null unique references public.agp_pessoas(id) on delete cascade,
  legacy_perfil_atleta_id uuid unique references public.perfis_atletas(id) on delete set null,
...
create table if not exists public.agp_credenciais_profissionais (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique nulls not distinct (pessoa_id, profissao, conselho, numero_registro)
);
...
declare
  ...
  v_tem_consentimento boolean := false;
  v_tem_linha_base boolean := false;
begin
  ...
  select exists(
    select 1 from public.agp_contas_acesso
    where pessoa_id = v_participante.pessoa_id and status = 'ativo'
  ) into v_tem_conta;

  select legacy_perfil_atleta_id into v_legacy_id
  from public.agp_perfis_esportivos
  where pessoa_id = v_participante.pessoa_id and status = 'ativo';

  v_tem_perfil := v_legacy_id is not null;

  if v_legacy_id is not null then
    select exists(
      select 1 from public.agp_consentimentos
      where atleta_id = v_legacy_id and revogado_em is null
    ) into v_tem_consentimento;

    select exists(
      select 1 from public.agp_linhas_base_atleta
      where atleta_id = v_legacy_id and projeto_id = v_participante.projeto_id
    ) into v_tem_linha_base;
  end if;
...
create policy agp_contas_owner_all on public.agp_contas_acesso for all using (public.agp_is_owner()) with check (public.agp_is_owner());
create policy agp_papeis_owner_all on public.agp_papeis_institucionais for all using (public.agp_is_owner()) with check (public.agp_is_owner());
create policy agp_perfis_owner_all on public.agp_perfis_esportivos for all using (public.agp_is_owner()) with check (public.agp_is_owner());
create policy agp_credenciais_owner_all on public.agp_credenciais_profissionais for all using (public.agp_is_owner()) with check (public.agp_is_owner());
create policy agp_responsaveis_owner_all on public.agp_responsaveis_atleta for all using (public.agp_is_owner()) with check (public.agp_is_owner());
create policy agp_auditoria_owner_read on public.agp_auditoria_participantes for select using (public.agp_is_owner());
